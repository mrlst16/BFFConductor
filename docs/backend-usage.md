...
## Two-Pronged Error Handling

BFFConductor covers the full error surface of an ASP.NET Core action through two complementary mechanisms that work together:

### Prong 1 — `OperationActionResult<T>` (Expected Failures)

For failures your service layer knows about and controls, use `OperationResult<T>.Fail(...)` and return an `OperationActionResult<T>` from your controller. The action result looks up the error code in the `ErrorMappingRegistry`, sets the correct HTTP status, and writes the `x-handle-message-as` header.

This path is for **anticipated, business-level errors** — validation failures, not-found, conflict, etc.

```csharp
public IActionResult Create([FromBody] CreateOrderRequest request)
{
    var result = _orderService.Create(request);          // returns OperationResult<T>
    return new OperationActionResult<Order>(result, _registry);  // converts to HTTP
}
```

### Prong 2 — `BffExceptionFilter` (Unexpected Exceptions)

For exceptions that escape the service layer, apply `[UseBffExceptionFilter]` to your controller. The filter catches any unhandled exception, matches it against the `ExceptionMappingRegistry`, and returns a structured `ApiResponse` error instead of a 500 stack trace.

This path is for **infrastructure and unexpected errors** — unauthorized access, null references, database failures, etc.

```csharp
[UseBffExceptionFilter]   // catches exceptions not handled by OperationResult
public class OrdersController : ControllerBase { }
```

### Together

Both prongs produce the same `ApiResponse` envelope and `x-handle-message-as` header, so the frontend handles them identically. The separation keeps your service layer free of HTTP concerns while ensuring no error escapes unformatted.

```
Action throws exception  →  BffExceptionFilter  →  ExceptionMappingRegistry  →  ApiResponse
Service returns Fail(...)  →  OperationActionResult  →  ErrorMappingRegistry  →  ApiResponse
```
